...
class HTTPHandler(object):
    ...
    def __init__(self, urls, headers=None, timeout=10, ssl_verify=True, silent=False, api_key="", access_token=""):
        ...
        if headers:
            ...
            if self.api_key != "":
                self.headers = {
                    'Authorization': f'MediaBrowser Client={jellypy.CONFIG.JELLYFIN_DEVICE_NAME}, Device=python-requests, DeviceId={jellypy.CONFIG.JELLYFIN_DEVICE_ID}, Version={jellypy.version.JELLYPY_VERSION}, Token={self.api_key}',
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                }
            elif self.access_token != "":
                self.headers = {
                    'Authorization': f'MediaBrowser Client={jellypy.CONFIG.JELLYFIN_DEVICE_NAME}, Device=python-requests, DeviceId={jellypy.CONFIG.JELLYFIN_DEVICE_ID}, Version={jellypy.version.JELLYPY_VERSION}, Token={self.access_token}',
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                }
        ...
        if self.api_key == "" and self.access_token == "":
            self.headers = {
                'Authorization': f'MediaBrowser Client={jellypy.CONFIG.JELLYFIN_DEVICE_NAME}, Device=python-requests, DeviceId={jellypy.CONFIG.JELLYFIN_DEVICE_ID}, Version={jellypy.version.JELLYPY_VERSION}',
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            }
    ...
    def make_request(self, uri=None, data=None, request_type='GET', output_format='json',
                     return_response=False, return_type=False, timeout=None, callback=None, json=None,
                     **request_kwargs):
        """
    ...
    def _http_requests_single(self, url):
        ...
        try:
            r = self._session.request(self.request_type, url, headers=self.headers, data=self.data,
                                      timeout=self.timeout, verify=self.ssl_verify, json=self.json,
                                      **self.request_kwargs)
            r.raise_for_status()
