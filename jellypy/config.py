...
_CONFIG_DEFINITIONS = {
    ...
    'JELLYFIN_USERNAME': (str, 'JELLYFIN', ''),
    'JELLYFIN_SESSION_ID': (str, 'JELLYFIN', ''),
    'JELLYFIN_PASSWORD': (str, 'JELLYFIN', ''),
    'JELLYFIN_ACCESS_TOKEN': (str, 'JELLYFIN', ''),
    'JELLYFIN_USER_ID': (str, 'JELLYFIN', ''),
    'JELLYFIN_URL_OVERRIDE': (str, 'JELLYFIN', ''),
    'JELLYFIN_USE_BIF': (int, 'JELLYFIN', 0),
    'JELLYFIN_UUID': (str, 'JELLYFIN', ''),
    'JELLYFIN_DEVICE_ID': (str, 'JELLYFIN', ''),
    'JELLYFIN_DEVICE_NAME': (str, 'JELLYFIN', 'JellyPy'),
    'JELLYFIN_TIMEOUT': (int, 'Advanced', 60),
    'JELLYFIN_PLATFORM': (str, 'JELLYFIN', ''),
    'JELLYFIN_VERSION': (str, 'JELLYFIN', ''),
    'JELLYFIN_UPDATE_CHANNEL': (str, 'JELLYFIN', 'stable'),
    'JELLYFIN_UPDATE_ARCH': (str, 'JELLYFIN', ''),
    ...
    'EXPORT_THREADS': (int, 'Advanced', 10),
    ...
    'GIT_USER': (str, 'General', 'NiNiyas'),
    'GIT_REPO': (str, 'General', 'JellyPy'),
    ...
    'HTTP_JELLYFIN_ADMIN': (int, 'General', 0),
    ...
    'NOTIFY_JELLYPY_UPDATE_REPEAT': (int, 'Monitoring', 0),
    'NOTIFY_TEXT_EVAL': (int, 'Advanced', 0),
    'JELLYPY_AUTO_UPDATE': (int, 'General', 0),
    'REFRESH_LIBRARIES_INTERVAL': (int, 'Monitoring', 12),
    'REFRESH_LIBRARIES_ON_STARTUP': (int, 'Monitoring', 0),
    'REFRESH_USERS_INTERVAL': (int, 'Monitoring', 12),
    'REFRESH_USERS_ON_STARTUP': (int, 'Monitoring', 0),
    ...
    'WEBSOCKET_MONITOR_PING_PONG': (int, 'Advanced', 1),
...
_DO_NOT_IMPORT_KEYS = [
    ...
    'ENABLE_HTTPS', 'HTTPS_CREATE_CERT', 'HTTPS_CERT', 'HTTPS_CERT_CHAIN', 'HTTPS_KEY', 'JWT_SECRET'
]
_DO_NOT_IMPORT_KEYS_DOCKER = [
    'JELLYPY_AUTO_UPDATE', 'GIT_REMOTE', 'GIT_BRANCH'
]
...
SETTINGS = [
    ...
    'JELLYFIN_USER_ID',
    'JELLYFIN_IS_REMOTE',
    'JELLYFIN_LOGS_FOLDER',
    'JELLYFIN_NAME',
    'JELLYFIN_PORT',
    'JELLYFIN_SSL',
    'JELLYFIN_UPDATE_CHANNEL',
    'JELLYFIN_UPDATE_CHECK_INTERVAL',
    'JELLYFIN_UPDATE_ARCH',
    'JELLYFIN_URL',
    'JELLYFIN_VERSION',
...
def import_tautulli_config(config=None, backup=False):
    if IS_IMPORTING:
        logger.warn("JellyPy Config :: Another JellyPy config is currently being imported. "
                    "Please wait until it is complete before importing another config.")
        return False

    if backup:
        # Make a backup of the current config first
        logger.info("JellyPy Config :: Creating a config backup before importing.")
        if not make_backup():
            logger.error("JellyPy Config :: Failed to import JellyPy config: failed to create config backup")
            return False
    ...
    try:
        ...
        logger.error("JellyPy Config :: Failed to import JellyPy config: error reading imported config file")
        return False

    logger.info("JellyPy Config :: Importing JellyPy config '%s'...", config)
    ...
    logger.info("JellyPy Config :: JellyPy config import complete.")
            ...
            for file_ in ini_files:
                if os.stat(file_).st_mtime < now - jellypy.CONFIG.BACKUP_DAYS * 86400:
                    try:
                        ...
                        logger.error("JellyPy Config :: Failed to delete %s from the backup folder: %s" % (file_, e))

    if backup_file in os.listdir(backup_folder):
        logger.debug("JellyPy Config :: Successfully backed up %s to %s" % (jellypy.CONFIG_FILE, backup_file))
        return True
    else:
        logger.error("JellyPy Config :: Failed to backup %s to %s" % (jellypy.CONFIG_FILE, backup_file))
        return False
...
class Config(object):
    ...
    def __init__(self, config_file, is_import=False):
        ...
        try:
            ...
            logger.error("JellyPy Config :: Error reading configuration file: %s", e)
            raise
    ...
    def write(self):
        ...
        # Write it to file
        logger.info("JellyPy Config :: Writing configuration to file")

        try:
            new_config.write()
        except IOError as e:
            logger.error("JellyPy Config :: Error writing configuration file: %s", e)
    ...
    def _upgrade(self):
        ...
        if self.CONFIG_VERSION == 5:
            self.MONITOR_JELLYFIN_UPDATES = 0
        ...
        if self.CONFIG_VERSION == 10:
            self.GIT_USER = 'NiNiyas'
            self.GIT_REPO = 'JellyPy'
