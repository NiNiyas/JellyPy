...
from jellypy import helpers, users
from jellypy.config import _BLACKLIST_KEYS, _WHITELIST_KEYS

# These settings are for file logging only
FILENAME = "jellypy.log"
FILENAME_API = "jellypy_api.log"
...
# Tautulli logger
logger = logging.getLogger("jellypy")
# Tautulli API logger
logger_api = logging.getLogger("jellypy_api")
...
def blacklist_config(config):
    blacklist = set()
    blacklist_keys = ['HOOK', 'APIKEY', 'KEY', 'PASSWORD', 'TOKEN', 'JELLYFIN_API_KEY', 'JELLYFIN_ACCESS_TOKEN', 'JELLYFIN_USER_ID', 'JELLYFIN_DEVICE_ID', 'JELLYFIN_URL', 'JELLYFIN_IP', 'JELLYFIN_PASSWORD']
...
class UsernameFilter(logging.Filter):
    ...
    def filter(self, record):
        if not jellypy.CONFIG.LOG_BLACKLIST_USERNAMES:
            return True

        if not jellypy._INITIALIZED:
            return True
