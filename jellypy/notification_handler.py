...
def set_hash_image_info(img=None, item_id=None, width=750, height=1000,
                        opacity=100, background='000000', blur=0, fallback=None,
                        add_to_db=True):
    if not item_id and not img:
        return fallback

    if item_id and not img:
        if fallback and fallback.startswith('art'):
            img = f'/Items/{item_id}/Images/Art'
        else:
            img = f'/Items/{item_id}/Images/Thumb'

    img_string = f'{jellypy.CONFIG.JELLYFIN_UUID}.{img}.{item_id}.{width}.{height}.{opacity}.{background}.{blur}'
