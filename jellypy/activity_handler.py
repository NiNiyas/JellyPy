...
class ActivityHandler(object):
    ...
    def is_valid_session(self):
        if 'sessionKey' in self.timeline:
            if str(self.timeline['sessionKey']):
                return True
    ...
    def get_metadata(self, skip_cache=False):
        ...
        pms_connect = jellyconnect.JellyConnect()
    ...
    def on_start(self):
        if self.is_valid_session():
            ...
            logger.debug("JellyPy ActivityHandler :: Session %s started by user %s (%s) with ratingKey %s (%s)%s."
    ...
    def on_stop(self, force_stop=False):
        if self.is_valid_session():
            logger.debug("JellyPy ActivityHandler :: Session %s %sstopped."
            ...
            if row_id:
                ...
                logger.debug("JellyPy ActivityHandler :: Removing sessionKey %s ratingKey %s from session queue"
    ...
    def on_pause(self, still_paused=False):
        if self.is_valid_session():
            if not still_paused:
                logger.debug("JellyPy ActivityHandler :: Session %s paused." % str(self.get_session_key()))
    ...
    def on_resume(self):
        if self.is_valid_session():
            logger.debug("JellyPy ActivityHandler :: Session %s resumed." % str(self.get_session_key()))
    ...
    def on_change(self):
        if self.is_valid_session():
            logger.debug(
                "JellyPy ActivityHandler :: Session %s has changed transcode decision." % str(self.get_session_key()))
    ...
    def on_buffer(self):
        if self.is_valid_session():
            logger.debug("JellyPy ActivityHandler :: Session %s is buffering." % self.get_session_key())
            ap = activity_processor.ActivityProcessor()
            ...
            logger.debug("JellyPy ActivityHandler :: Session %s buffer count is %s." %
            ...
            if buffer_last_triggered:
                logger.debug("JellyPy ActivityHandler :: Session %s buffer last triggered at %s." %
    ...
    def on_error(self):
        if self.is_valid_session():
            logger.debug("JellyPy ActivityHandler :: Session %s encountered an error." % str(self.get_session_key()))
    ...
    def process(self):
        if self.is_valid_session():
            ...
            if db_session:
                ...
                schedule_callback(f'session_key-{self.get_session_key()}',
                                  func=force_stop_stream,
                                  args=[self.get_session_key(), db_session['full_title'], db_session['user']],
                                  minutes=5)
                ...
                if not db_session['watched'] and this_state != 'buffering':
                    ...
                    watched_percent = {
                        'movie': jellypy.CONFIG.MOVIE_WATCHED_PERCENT,
                    ...
                    if progress_percent >= watched_percent.get(db_session['media_type'], 101):
                        logger.debug("JellyPy ActivityHandler :: Session %s watched."
        ...
        if self.is_item():
            ...
            if media_type and section_id > 0 and state_type == 0 and metadata_state == 'created':
                ...
                if media_type in ('episode', 'track'):
                    ...
                    logger.debug("JellyPy TimelineHandler :: Library item '%s' (%s, grandparent %s) "
                    ...
                    logger.debug("JellyPy TimelineHandler :: Library item '%s' (%s , parent %s) "
                                 "added to recently added queue."
                    ...
                    logger.debug("JellyPy TimelineHandler :: Library item '%s' (%s) "
                                 "added to recently added queue."
            ...
            # An item was deleted, make sure it is removed from the queue
                ...
                if rating_key in RECENTLY_ADDED_QUEUE and not RECENTLY_ADDED_QUEUE[rating_key]:
                    logger.debug("JellyPy TimelineHandler :: Library item %s "
    ...
    def process(self):
        ...
        if server_response:
            ...
            if server_response['mapping_state'] == 'waiting':
                logger.warn("JellyPy ReachabilityHandler :: Remote access waiting for port mapping.")

            elif jellypy.PLEX_REMOTE_ACCESS_UP is not False and server_response['reason']:
                logger.warn("JellyPy ReachabilityHandler :: Remote access failed: %s" % server_response['reason'])
                logger.info("JellyPy ReachabilityHandler :: Plex remote access is down.")

                jellypy.PLEX_REMOTE_ACCESS_UP = False

                if not ACTIVITY_SCHED.get_job('on_extdown'):
                    logger.debug(
                        "JellyPy ReachabilityHandler :: Scheduling remote access down callback in %d seconds.",
                ...
                logger.info("JellyPy ReachabilityHandler :: Plex remote access is back up.")

                jellypy.PLEX_REMOTE_ACCESS_UP = True

                if ACTIVITY_SCHED.get_job('on_extdown'):
                    logger.debug("JellyPy ReachabilityHandler :: Cancelling scheduled remote access down callback.")
...
def force_stop_stream(session_key, title, user):
    ...
    if row_id:
        jellypy.NOTIFY_QUEUE.put({'stream_data': session.copy(), 'notify_action': 'on_stop'})

        # If session is written to the database successfully, remove the session from the session table
        logger.info(
            "JellyPy ActivityHandler :: Removing stale stream with sessionKey %s ratingKey %s from session queue"
        ...
        if session['write_attempts'] < jellypy.CONFIG.SESSION_DB_WRITE_ATTEMPTS:
            logger.warn(
                "JellyPy ActivityHandler :: Failed to write stream with sessionKey %s ratingKey %s to the database. " \
...
def clear_recently_added_queue(rating_key, title):
    logger.debug("JellyPy TimelineHandler :: Starting callback for library item '%s' (%s) after delay.",
                 title, str(rating_key))
...
def on_created(rating_key, **kwargs):
    ...
    logger.debug("JellyPy TimelineHandler :: Library item '%s' (%s) added to Plex.",
    ...
    if metadata:
        ...
        #     logger.debug("JellyPy TimelineHandler :: Library item %s added more than 24 hours ago. Not notifying."
        #                  % str(rating_key))
        #     notify = False
        ...
        if 'child_keys' not in kwargs:
            if data_factory.get_recently_added_item(rating_key):
                logger.debug("JellyPy TimelineHandler :: Library item %s added already. Not notifying again."
                             % str(rating_key))
                notify = False
        ...
        logger.error("JellyPy TimelineHandler :: Unable to retrieve metadata for rating_key %s" % str(rating_key))
...
def delete_metadata_cache(session_key):
    try:
        ...
        logger.error("JellyPy ActivityHandler :: Failed to remove metadata cache file (sessionKey %s): %s"
                     % (session_key, e))
