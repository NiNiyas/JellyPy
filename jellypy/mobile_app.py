...
def add_mobile_device(device_id=None, device_name=None, device_token=None,
    ...
    try:
        ...
        logger.warn("JellyPy MobileApp :: Failed to register mobile device in the database: %s." % e)
        return

    if result == 'insert':
        logger.info("JellyPy MobileApp :: Registered mobile device '%s' in the database." % device_name)
    else:
        logger.info("JellyPy MobileApp :: Re-registered mobile device '%s' in the database." % device_name)
...
def get_mobile_device_config(mobile_device_id=None):
    if str(mobile_device_id).isdigit():
        ...
        logger.error(
            "JellyPy MobileApp :: Unable to retrieve mobile device config: invalid mobile_device_id %s." % mobile_device_id)
        return None
...
def set_mobile_device_config(mobile_device_id=None, **kwargs):
    if str(mobile_device_id).isdigit():
        ...
        logger.error(
            "JellyPy MobileApp :: Unable to set exisiting mobile device: invalid mobile_device_id %s." % mobile_device_id)
        return False
    ...
    try:
        ...
        logger.info("JellyPy MobileApp :: Updated mobile device agent: mobile_device_id %s." % mobile_device_id)
        blacklist_logger()
        return True
    except Exception as e:
        logger.warn("JellyPy MobileApp :: Unable to update mobile device: %s." % e)
        return False
...
def delete_mobile_device(mobile_device_id=None, device_id=None):
    ...
    if mobile_device_id:
        logger.debug("JellyPy MobileApp :: Deleting mobile_device_id %s from the database." % mobile_device_id)
        result = db.action('DELETE FROM mobile_devices WHERE id = ?', args=[mobile_device_id])
        return True
    elif device_id:
        logger.debug("JellyPy MobileApp :: Deleting device_id %s from the database." % device_id)
...
def set_official(device_id, onesignal_id):
    ...
    try:
        ...
        logger.warn("JellyPy MobileApp :: Failed to set official flag for device: %s." % e)
        return
...
def set_last_seen(device_token=None):
    ...
    try:
        ...
        logger.warn("JellyPy MobileApp :: Failed to set last_seen time for device: %s." % e)
        return
...
def validate_onesignal_id(onesignal_id):
    ...
    logger.info("JellyPy MobileApp :: Validating OneSignal ID")
    try:
        r = requests.get('https://onesignal.com/api/v1/players/{}'.format(onesignal_id), headers=headers, params=params)
        status_code = r.status_code
        logger.info("JellyPy MobileApp :: OneSignal ID validation returned status code %s", status_code)
        return int(status_code == 200)
    except Exception as e:
        logger.warn("JellyPy MobileApp :: Failed to validate OneSignal ID: %s." % e)
        return -1
