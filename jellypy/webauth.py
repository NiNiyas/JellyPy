...
def plex_user_login(token=None, headers=None):
    ...
    if token:
        plex_tv = Jellyfin(token=token, headers=headers)
    ...
    if user_token and user_id:
        ...
        if user_id != str(user_details['user_id']):
            ...
            return None
        elif jellypy.CONFIG.HTTP_JELLYFIN_ADMIN and user_details['is_admin']:
            # Plex admin login
        ...
        plex_tv = Jellyfin(token=user_token, headers=headers)
        ...
        if server_token:
            ...
            try:
                logger.debug("JellyPy WebAuth :: Registering token for user '%s' in the database."
                ...
                if result:
                    ...
                    logger.warn("JellyPy WebAuth :: Unable to register user '%s' in database."
                                % user_details['username'])
                    return None
            except Exception as e:
                logger.warn("JellyPy WebAuth :: Unable to register user '%s' in database: %s."
                            % (user_details['username'], e))
                return None
        else:
            logger.warn("JellyPy WebAuth :: Unable to retrieve Plex.tv server token for user '%s'."
                        % user_details['username'])
            return None

    elif token:
        logger.warn("JellyPy WebAuth :: Unable to retrieve Plex.tv user token for Plex OAuth.")
        return None
...
def check_credentials(username=None, password=None, token=None, admin_login='0', headers=None):
    ...
    if username and password:
        if jellypy.CONFIG.HTTP_PASSWORD:
            ...
            if username == jellypy.CONFIG.HTTP_USERNAME and check_hash(password, jellypy.CONFIG.HTTP_PASSWORD):
                return True, user_details, 'admin'

    if jellypy.CONFIG.HTTP_JELLYFIN_ADMIN or (not admin_login == '1' and jellypy.CONFIG.ALLOW_GUEST_ACCESS):
...
def get_jwt_token():
    jwt_cookie = str(JWT_COOKIE_NAME + jellypy.CONFIG.JELLYFIN_UUID)
...
def check_jwt_token():
    ...
    if jwt_token:
        try:
            payload = jwt.decode(
                jwt_token, jellypy.CONFIG.JWT_SECRET, leeway=timedelta(seconds=10), algorithms=[JWT_ALGORITHM]
            )
...
class AuthController(object):
    ...
    def on_login(self, username=None, user_id=None, user_group=None, success=False, oauth=False,
        ...
        if success:
            ...
            logger.debug("JellyPy WebAuth :: %s user '%s' logged into JellyPy using %s login."
    ...
    def on_logout(self, username, user_group, jwt_token=None):
        ...
        logger.debug("JellyPy WebAuth :: %s user '%s' logged out of JellyPy." % (user_group.capitalize(), username))
    ...
    def signin(self, username=None, password=None, token=None, remember_me='0', admin_login='0', *args, **kwargs):
        ...
        if rate_limit:
            logger.debug("JellyPy WebAuth :: Too many incorrect login attempts from '%s'." % ip_address)
        ...
        if valid_login:
            ...
            jwt_token = jwt.encode(payload, jellypy.CONFIG.JWT_SECRET, algorithm=JWT_ALGORITHM)
            ...
            return {'status': 'success', 'token': jwt_token, 'uuid': jellypy.CONFIG.JELLYFIN_UUID}

        elif admin_login == '1' and username:
            self.on_login(username=username)
            logger.debug("JellyPy WebAuth :: Invalid admin login attempt from '%s'." % username)
            cherrypy.response.status = 401
            return error_message

        elif username:
            self.on_login(username=username)
            logger.debug("JellyPy WebAuth :: Invalid user login attempt from '%s'." % username)
            cherrypy.response.status = 401
            return error_message

        elif token:
            self.on_login(username='Plex OAuth', oauth=True)
            logger.debug("JellyPy WebAuth :: Invalid Plex OAuth login attempt.")
            cherrypy.response.status = 401
            return error_message

    @cherrypy.expose
    def redirect(self, redirect_uri='', *args, **kwargs):
        root = jellypy.HTTP_ROOT.rstrip('/')
        if redirect_uri.startswith(root):
            redirect_uri = redirect_uri[len(root):]
        raise cherrypy.HTTPRedirect(jellypy.HTTP_ROOT + redirect_uri.strip('/'))
