    ...
    def get_home_stats(self, grouping=None, time_range=30, stats_type='plays',
        ...
        for stat in stats_cards:
            if stat == 'top_movies':
                ...
                for item in result:
                    row = {
                        'title': item['original_title'] or item['grandparent_title'],
                        'year': item['year'],
                        'total_plays': item['total_plays'],
                        'total_duration': item['total_duration'],
                        'users_watched': '',
                ...
                try:
                    ...
                    logger.warn(
                        "Tautulli DataFactory :: Unable to execute database query for get_home_stats: top_libraries: %s." % e)
                    return None
    ...
    def get_library_stats(self, library_cards=[]):
        ...
        try:
            query = "SELECT ls.id, ls.section_id, ls.section_name, ls.section_type, ls.thumb AS library_thumb, " \
                    ...
                    "GROUP BY ls.id " \
                    "ORDER BY ls.section_type, ls.count DESC, ls.parent_count DESC, ls.child_count DESC" % ','.join(
                ["'{}'".format(value) for value in library_cards])
            result = monitor_db.select(query)
        except Exception as e:
            logger.warn(f"JellyPy DataFactory :: Unable to execute database query for get_library_stats: {e}.")
            return None
