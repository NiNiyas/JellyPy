...
def validate_database(database=None):
    try:
        ...
        logger.error("JellyPy Database :: Invalid database specified: %s", e)
        return 'Invalid database specified'
    except Exception as e:
        logger.error("JellyPy Database :: Uncaught exception: %s", e)
...
def import_tautulli_db(database=None, method=None, backup=False):
    if IS_IMPORTING:
        logger.warn("JellyPy Database :: Another JellyPy database is currently being imported. "
    ...
    if not db_validate == 'success':
        logger.error("JellyPy Database :: Failed to import JellyPy database: %s", db_validate)
        return False

    if method not in ('merge', 'overwrite'):
        logger.error("JellyPy Database :: Failed to import JellyPy database: invalid import method '%s'", method)
        return False

    if backup:
        # Make a backup of the current database first
        logger.info("JellyPy Database :: Creating a database backup before importing.")
        if not make_backup():
            logger.error("JellyPy Database :: Failed to import JellyPy database: failed to create database backup")
            return False

    logger.info("JellyPy Database :: Importing JellyPy database '%s' with import method '%s'...", database, method)
    ...
    logger.info("JellyPy Database :: Import JellyPy database version: %s", import_db_version)
    ...
    if method == 'merge':
        logger.info("JellyPy Database :: Creating temporary database tables to re-index grouped session history.")
    ...
    for table in tables:
        ...
        logger.info("JellyPy Database :: Importing database table '%s'.", table_name)
    ...
    if method == 'merge':
        for table_name, columns in sorted(table_columns.items()):
            ...
            logger.info("JellyPy Database :: Removing duplicate rows from database table '%s'.", table_name)
        ...
        logger.info("JellyPy Database :: Deleting temporary database tables.")
        for table_name in session_history_tables:
            db.action('DROP TABLE {table}_copy'.format(table=table_name))

    vacuum()

    logger.info("JellyPy Database :: JellyPy database import complete.")
    set_is_importing(False)

    logger.info("JellyPy Database :: Deleting cached database: %s", database)
    os.remove(database)
...
def clear_table(table=None):
    if table:
        ...
        logger.debug("JellyPy Database :: Clearing database table '%s'." % table)
        try:
            monitor_db.action('DELETE FROM %s' % table)
            vacuum()
            return True
        except Exception as e:
            logger.error("JellyPy Database :: Failed to clear database table '%s': %s." % (table, e))
            return False


def delete_sessions():
    logger.info("JellyPy Database :: Clearing temporary sessions from database.")
    return clear_table('sessions')


def delete_recently_added():
    logger.info("JellyPy Database :: Clearing recently added items from database.")
    return clear_table('recently_added')


def delete_exports():
    logger.info("JellyPy Database :: Clearing exported items from database.")
...
def delete_rows_from_table(table, row_ids):
    ...
    if row_ids:
        logger.info("JellyPy Database :: Deleting row ids %s from %s database table", row_ids, table)
        ...
        try:
            ...
            logger.error("JellyPy Database :: Failed to delete rows from %s database table: %s" % (table, e))
            return False
...
def delete_user_history(user_id=None):
    if str(user_id).isdigit():
        ...
        logger.info("JellyPy Database :: Deleting all history for user_id %s from database." % user_id)
...
def delete_library_history(section_id=None):
    if str(section_id).isdigit():
        ...
        logger.info("JellyPy Database :: Deleting all history for library section_id %s from database." % section_id)
        return delete_session_history_rows(row_ids=row_ids)
...
def vacuum():
    ...
    logger.info("JellyPy Database :: Vacuuming database.")
    try:
        monitor_db.action('VACUUM')
    except Exception as e:
        logger.error("JellyPy Database :: Failed to vacuum database: %s" % e)


def optimize():
    monitor_db = MonitorDatabase()

    logger.info("JellyPy Database :: Optimizing database.")
    try:
        monitor_db.action('PRAGMA optimize')
    except Exception as e:
        logger.error("JellyPy Database :: Failed to optimize database: %s" % e)
...
def make_backup(cleanup=False, scheduler=False):
    ...
    if not integrity:
        corrupt = '.corrupt'
        jellypy.NOTIFY_QUEUE.put({'notify_action': 'on_jellypydbcorrupt'})
            ...
            for file_ in db_files:
                if os.stat(file_).st_mtime < now - jellypy.CONFIG.BACKUP_DAYS * 86400:
                    try:
                        ...
                        logger.error("JellyPy Database :: Failed to delete %s from the backup folder: %s" % (file_, e))

    if backup_file in os.listdir(backup_folder):
        logger.debug("JellyPy Database :: Successfully backed up %s to %s" % (db_filename(), backup_file))
        return True
    else:
        logger.error("JellyPy Database :: Failed to backup %s to %s" % (db_filename(), backup_file))
        return False
            ...
            while attempts < 5:
                try:
                    ...
                    if "unable to open database file" in e or "database is locked" in e:
                        logger.warn("JellyPy Database :: Database Error: %s", e)
                        attempts += 1
                        time.sleep(1)
                    else:
                        logger.error("JellyPy Database :: Database error: %s", e)
                        raise

                except sqlite3.DatabaseError as e:
                    logger.error("JellyPy Database :: Fatal Error executing %s :: %s", query, e)
                    raise
    ...
    def upsert(self, table_name, value_dict, key_dict):
        ...
        if self.connection.total_changes == changes_before:
            ...
            try:
                ...
                logger.info("JellyPy Database :: Queries failed: %s and %s", update_query, insert_query)
