...
class ActivityProcessor(object):
    ...
    def write_session_history(self, session=None, import_metadata=None, is_import=False, import_ignore_interval=0):
        ...
        if session:
            ...
            if str(session['rating_key']) and session['media_type'] in ('movie', 'episode', 'track'):
                logging_enabled = True
            else:
                logger.debug("JellyPy ActivityProcessor :: Session %s ratingKey %s not logged. "
            ...
            if not is_import and jellypy.CONFIG.LOGGING_IGNORE_INTERVAL:
                if (session['media_type'] == 'movie' or session['media_type'] == 'episode') and \
                    ...
                    logging_enabled = False
                    logger.debug("JellyPy ActivityProcessor :: Play duration for session %s ratingKey %s is %s secs "
            ...
            if not is_import and session['media_type'] == 'track':
                ...
                if (session['media_type'] == 'movie' or session['media_type'] == 'episode') and \
                    ...
                    logging_enabled = False
                    logger.debug(
                        "JellyPy ActivityProcessor :: Play duration for ratingKey %s is %s secs which is less than %s "
            ...
            if not is_import and not user_details['keep_history']:
                logging_enabled = False
                logger.debug("JellyPy ActivityProcessor :: History logging for user '%s' is disabled." % user_details[
                    'username'])
            elif not is_import and not library_details['keep_history']:
                logging_enabled = False
                logger.debug(
                    "JellyPy ActivityProcessor :: History logging for library '%s' is disabled." % library_details[
                        'section_name'])
            ...
            if logging_enabled:
                ...
                if not is_import:
                    logger.debug(
                        "JellyPy ActivityProcessor :: Fetching metadata for item ratingKey %s" % session['rating_key'])
                ...
                # logger.debug("JellyPy ActivityProcessor :: Attempting to write sessionKey %s to session_history table..."
                #              % session['session_key'])
                keys = {'id': None}
                ...
                # logger.debug("JellyPy ActivityProcessor :: Writing sessionKey %s session_history transaction..."
                #              % session['session_key'])
                ...
                # logger.debug("JellyPy ActivityProcessor :: Successfully written history item, last id for session_history is %s"
                #              % last_id)

                # Write the session_history_media_info table

                # logger.debug("JellyPy ActivityProcessor :: Attempting to write to sessionKey %s session_history_media_info table..."
                #              % session['session_key'])
                keys = {'id': last_id}
                ...
                # logger.debug("JellyPy ActivityProcessor :: Writing sessionKey %s session_history_media_info transaction..."
                #              % session['session_key'])
                ...
                # logger.debug("JellyPy ActivityProcessor :: Attempting to write to sessionKey %s session_history_metadata table..."
                #              % session['session_key'])
                keys = {'id': last_id}
                values = {'rating_key': session['rating_key'],
                          'parent_rating_key': session['parent_rating_key'],
                ...
                # logger.debug("JellyPy ActivityProcessor :: Writing sessionKey %s session_history_metadata transaction..."
                #              % session['session_key'])
