...
from jellypy.webauth import AuthController, requireAuth, member_of, check_auth, get_jwt_token
...
def serve_template(templatename, **kwargs):
    ...
    template_dir = os.path.join(str(interface_dir), jellypy.CONFIG.INTERFACE)
...
class BaseRedirect(object):
    ...
    def status(self, *args, **kwargs):
        ...
        raise cherrypy.HTTPRedirect(jellypy.HTTP_ROOT + 'status' + path + query)
...
class WebInterface(object):
    ...
    def __init__(self):
        self.interface_dir = os.path.join(str(jellypy.PROG_DIR), 'data/')

    @cherrypy.expose
    @requireAuth()
    def index(self, **kwargs):
        if jellypy.CONFIG.FIRST_RUN_COMPLETE:
            raise cherrypy.HTTPRedirect(jellypy.HTTP_ROOT + "home")
        else:
    ...
    def welcome(self, **kwargs):
        config = {
            ...
            "jellyfin_username": jellypy.CONFIG.JELLYFIN_USERNAME,
            "jellyfin_password": jellypy.CONFIG.JELLYFIN_PASSWORD,
    ...
    def discover(self, access_token=None, hostname=None, port=None, ssl=0, user_id=None, session_id=None, **kwargs):
        """ Gets server details after logging in to Jellyfin and writes to config.
        ...
        if request:
            jellypy.CONFIG.__setattr__('JELLYFIN_NAME', request['ServerName'])
            jellypy.CONFIG.__setattr__('JELLYFIN_VERSION', request['Version'])
            jellypy.CONFIG.__setattr__('JELLYFIN_PLATFORM', request['OperatingSystem'])
            jellypy.CONFIG.__setattr__('JELLYFIN_URL', f"{scheme}://{hostname}:{port}")
            jellypy.CONFIG.__setattr__('JELLYFIN_IDENTIFIER', request['Id'])
            jellypy.CONFIG.__setattr__('JELLYFIN_ACCESS_TOKEN', access_token)
            jellypy.CONFIG.__setattr__('JELLYFIN_USER_ID', user_id)
            jellypy.CONFIG.__setattr__('JELLYFIN_SESSION_ID', session_id)
            # jellypy.CONFIG.__setattr__('JELLYFIN_UPDATE_ARCH', request['SystemArchitecture'].lower())
            # jellypy.CONFIG.__setattr__('JELLYFIN_LOGS_FOLDER', request['LogPath'])
            jellypy.CONFIG.write()

    # https://github.com/jellyfin/mopidy-jellyfin/blob/master/mopidy_jellyfin/ws_client.py#L112
    # Thanks @mcarlton:nerdyredneck.net and @nielsvanvelzen:matrix.org
    @cherrypy.expose
    @cherrypy.tools.json_out()
    @requireAuth(member_of("admin"))
    def post_capabilities(self, access_token=None, hostname=None, port=None, ssl=0, session_id=None):
        ...
        data = {
            ...
            "DeviceProfile": {
                "Id": jellypy.CONFIG.JELLYFIN_DEVICE_ID
            },
            "UserId": jellypy.CONFIG.JELLYFIN_USER_ID,
        }

        ssl = helpers.bool_true(ssl)
        scheme = 'https' if ssl else 'http'
        url = '{scheme}://{hostname}:{port}'.format(scheme=scheme, hostname=hostname, port=port)
        uri = f'/Sessions/Capabilities/Full?id={session_id}'

        request_handler = http_handler.HTTPHandler(urls=url, ssl_verify=False, access_token=access_token)
        request = request_handler.make_request(uri=uri, request_type='POST', json=data)

        if request == "success":
            logger.info("Successfully updated capabilities in Jellyfin Server.")
        else:
            logger.error("Unable to update capabilities in Jellyfin Server.")

    @cherrypy.expose
    @cherrypy.tools.json_out()
    @requireAuth(member_of("admin"))
    def check_jellyfin_token(self, hostname=None, port=None, ssl=0, api_key=None, **kwargs):
        """ Gets server details after logging in to Jellyfin and writes to config.
        ...
        request_handler = http_handler.HTTPHandler(urls=url, ssl_verify=False, api_key=api_key)
        request = request_handler.make_request(uri=uri, request_type='GET', output_format='json')
    ...
    @requireAuth(member_of("admin"))
    # @addtoapi("get_server_list")
    def discover_private_info(self, hostname=None, port=None, ssl=0, api_key=None, **kwargs):
        """ Gets server details after logging in to Jellyfin and writes to config.
    ...
    @cherrypy.tools.json_out()
    @requireAuth(member_of("admin"))
    def config_write(self, _key, value):
        jellypy.CONFIG.__setattr__(_key, value)
        jellypy.CONFIG.write()
    ...
    def get_date_formats(self, **kwargs):
        """ Get the date and time formats used by Tautulli.
            ...
            ```
        """
        if jellypy.CONFIG.DATE_FORMAT:
            date_format = jellypy.CONFIG.DATE_FORMAT
        else:
            date_format = 'YYYY-MM-DD'
        if jellypy.CONFIG.TIME_FORMAT:
    ...
    def get_current_activity(self, **kwargs):
        ...
        result = jellyfin_connect.get_current_activity()

        print(f"webserve: get_current_activity: {result}")
    ...
    def terminate_session(self, session_key='', session_id='', message='', **kwargs):
        ...
        jellyfin_connect = jellyconnect.JellyConnect()
        result = jellyfin_connect.terminate_session(session_key=session_key, session_id=session_id, message=message)

        if result is True:
            return {'result': 'success', 'message': 'Session terminated.'}
        elif result:
            return {'result': 'error', 'message': f'Failed to terminate session: {result}.'}
        else:
    ...
    def library_stats(self, **kwargs):
        ...
        library_cards = jellypy.CONFIG.HOME_LIBRARY_CARDS
    ...
    def get_recently_added(self, count='0', media_type='', **kwargs):
        try:
            jellyfin_connect = jellyconnect.JellyConnect()
            result = jellyfin_connect.get_recently_added_details(count=count, media_type=media_type)
        except IOError as e:
    ...
    def library(self, section_id=None, **kwargs):
        ...
        config = {
            "get_file_sizes": jellypy.CONFIG.GET_FILE_SIZES,
    ...
    def library_recently_added(self, section_id=None, limit='10', **kwargs):
        ...
        if section_id:
            jellyfin_connect = jellyconnect.JellyConnect()
            result = jellyfin_connect.get_recently_added_details(section_id=section_id, count=limit)
        else:
            result = None
    ...
    def real_pms_image_proxy(self, img=None, rating_key=None, width=750, height=1000, opacity=100, background='000000',
                             blur=0, img_format='png', fallback=None, refresh=False, clip=False, **kwargs):
        ...
        if isinstance(img, str) and img.startswith('interfaces/default/images'):
            fp = os.path.join(jellypy.PROG_DIR, 'data', img)
            return serve_file(path=fp, content_type='image/png')

        if not img and not rating_key:
            if fallback in common.DEFAULT_IMAGES:
                fbi = common.DEFAULT_IMAGES[fallback]
                fp = os.path.join(jellypy.PROG_DIR, 'data', fbi)
                return serve_file(path=fp, content_type='image/jpeg')
        ...
        if rating_key and not img:
            if fallback and fallback.startswith('art'):
                img = f'/Items/{rating_key}/Art'
            else:
                img = f'/Items/{rating_key}/Thumb'

        img_hash = notification_handler.set_hash_image_info(img=img, item_id=rating_key, width=width, height=height,
                                                            opacity=opacity, background=background, blur=blur,
                                                            fallback=fallback, add_to_db=return_hash)

        if return_hash:
            return {'img_hash': img_hash}

        fp = f'{img_hash}.{img_format}'  # we want to be able to preview the thumbs
        c_dir = os.path.join(jellypy.CONFIG.CACHE_DIR, 'images')
        ...
        try:
            if not jellypy.CONFIG.CACHE_IMAGES or refresh or 'indexes' in img:
                raise NotFound
            ...
            try:
                ...
                jelly_connect.request_handler._silent = True
                result = jelly_connect.get_image(img=img, width=width, height=height, opacity=opacity,
                                                 background=background, blur=blur, img_format=img_format, clip=clip,
                                                 refresh=refresh)

                if result and result[0]:
                    cherrypy.response.headers['Content-type'] = result[1]
                    if jellypy.CONFIG.CACHE_IMAGES and 'indexes' not in img:
    ...
    def get_library_media_info(self, section_id=None, section_type=None, rating_key=None, refresh='', **kwargs):
        ...
        library_data = libraries.Libraries()
        result = library_data.get_datatables_media_info(section_id=section_id, section_type=section_type,
                                                        rating_key=rating_key,
                                                        refresh=refresh, kwargs=kwargs)
    ...
    def get_media_info_file_sizes(self, section_id=None, rating_key=None, **kwargs):
        ...
        if (section_id and section_id not in section_ids) or (rating_key and rating_key not in rating_keys):
            ...
            jellypy.CONFIG.GET_FILE_SIZES_HOLD = {'section_ids': list(section_ids), 'rating_keys': list(rating_keys)}
    ...
    def user(self, user_id=None, **kwargs):
        ...
        if user_id:
            try:
                ...
                logger.warn(f"Unable to retrieve user details for user_id {user_id}")
                    ...
                    {"draw": 1,
                     ...
                     "data":
                        [{"browser": "Safari 7.0.3",
                          ...
                          "friendly_name": "Jon Snow",
                          "host": "http://jellypy.castleblack.com",
                          "ip_address": "xxx.xxx.xxx.xxx",
                          "os": "Mac OS X",
                          "row_id": 1,
    ...
    def get_log(self, logfile='', **kwargs):
        ...
        with open(os.path.join(jellypy.CONFIG.LOG_DIR, filename), 'r', encoding='utf-8') as f:
    ...
    @requireAuth(member_of("admin"))
    @addtoapi()
    def get_jellyfin_log(self, logfile='', **kwargs):
        ...
        try:
            ...
            logger.warn(f"Unable to retrieve Jellyfin log file '{logfile}'.")
            return []
    ...
    def delete_logs(self, logfile='', **kwargs):
        ...
        try:
            open(os.path.join(jellypy.CONFIG.LOG_DIR, filename), 'w').close()
            result = 'success'
    ...
    def toggleVerbose(self, **kwargs):
        jellypy.VERBOSE = not jellypy.VERBOSE

        jellypy.CONFIG.__setattr__('VERBOSE_LOGS', jellypy.VERBOSE)
        jellypy.CONFIG.write()

        logger.initLogger(console=not jellypy.QUIET, log_dir=jellypy.CONFIG.LOG_DIR, verbose=jellypy.VERBOSE)
        logger.info("Verbose toggled, set to %s", jellypy.VERBOSE)
        logger.debug("If you read this message, debug logging is available")
        raise cherrypy.HTTPRedirect(jellypy.HTTP_ROOT + "logs")
    ...
    def settings(self, **kwargs):
        ...
        for setting in config.SETTINGS:
            settings_dict[setting.lower()] = getattr(jellypy.CONFIG, setting)

        for setting in config.CHECKED_SETTINGS:
            settings_dict[setting.lower()] = checked(getattr(jellypy.CONFIG, setting))
    ...
    def configUpdate(self, **kwargs):
        ...
        if kwargs.get('launch_startup') != jellypy.CONFIG.LAUNCH_STARTUP or \
                kwargs.get('launch_browser') != jellypy.CONFIG.LAUNCH_BROWSER:
        ...
        if kwargs.get('check_github') != jellypy.CONFIG.CHECK_GITHUB or \
                kwargs.get('check_github_interval') != str(jellypy.CONFIG.CHECK_GITHUB_INTERVAL) or \
                ...
                kwargs.get('backup_interval') != str(jellypy.CONFIG.BACKUP_INTERVAL):
            reschedule = True
        ...
        if kwargs.get('jellyfin_ssl') != str(jellypy.CONFIG.JELLYFIN_SSL) or \
                kwargs.get('jellyfin_is_remote') != str(jellypy.CONFIG.JELLYFIN_IS_REMOTE):
            server_changed = True
        ...
        if kwargs.get('enable_https') and kwargs.get('https_create_cert'):
            if kwargs.get('https_domain') != jellypy.CONFIG.HTTPS_DOMAIN or \
                    kwargs.get('https_ip') != jellypy.CONFIG.HTTPS_IP or \
                    kwargs.get('https_cert') != jellypy.CONFIG.HTTPS_CERT or \
                    kwargs.get('https_key') != jellypy.CONFIG.HTTPS_KEY:
                https_changed = True
        ...
        jellypy.CONFIG.process_kwargs(kwargs)
        ...
        if https_changed:
            create_https_certificates(jellypy.CONFIG.HTTPS_CERT, jellypy.CONFIG.HTTPS_KEY)
    ...
    @cherrypy.tools.json_out()
    @requireAuth(member_of("admin"))
    def get_server_resources(self, return_presence=False, return_server=False, return_info=False, **kwargs):
        if not return_presence and not return_info:
            logger.info("JellyPy :: Requesting resources for server...")

        server = {
            'jellyfin_name': jellypy.CONFIG.JELLYFIN_NAME,
            'jellyfin_version': jellypy.CONFIG.JELLYFIN_VERSION,
            'jellyfin_platform': jellypy.CONFIG.JELLYFIN_PLATFORM,
            'jellyfin_ip': jellypy.CONFIG.JELLYFIN_IP,
            'jellyfin_port': jellypy.CONFIG.JELLYFIN_PORT,
            'jellyfin_ssl': jellypy.CONFIG.JELLYFIN_SSL,
        ...
        if kwargs:
            ...
            for k in ['jellyfin_ssl', 'jellyfin_is_remote']:
                server[k] = int(server[k])

        if server['jellyfin_ssl']:
            scheme = 'https'
        else:
            scheme = 'http'

        fallback_url = '{scheme}://{hostname}:{port}'.format(scheme=scheme, hostname=server['jellyfin_ip'],
                                                             port=server['jellyfin_port'])

        if not server['jellyfin_url']:
            server['jellyfin_url'] = fallback_url
            logger.warn("JellyPy :: Unable to retrieve server URL. Using fallback value without SSL.")
        else:
            server['jellyfin_url'] = fallback_url
            logger.info("JellyPy:: Using user-defined URL.")

        if return_server:
            return server

        logger.info("JellyPy :: Selected server: %s (%s) (%s - Version %s)",
                    server['jellyfin_name'], server['jellyfin_url'], server['jellyfin_platform'],
                    server['jellyfin_version'])
    ...
    def backup_config(self, **kwargs):
        """ Creates a manual backup of the jellypy.db file """
    ...
    def get_server_update_params(self, **kwargs):
        ...
        #
        # update_channel = jellyconnect.JellyConnect().get_server_update_channel()
        #
        # return {'plexpass': plexpass,
        #         'jellyfin_platform': common.JELLYFIN_PLATFORM_NAME_OVERRIDES.get(
        #             jellypy.CONFIG.JELLYFIN_PLATFORM, jellypy.CONFIG.JELLYFIN_PLATFORM),
        #         'jellyfin_update_channel': jellypy.CONFIG.JELLYFIN_UPDATE_CHANNEL,
        #         'jellyfin_update_distro': jellypy.CONFIG.JELLYFIN_UPDATE_DISTRO,
        #         'jellyfin_update_distro_build': jellypy.CONFIG.JELLYFIN_UPDATE_DISTRO_BUILD,
    ...
    def facebook_retrieve_token(self, **kwargs):
        if jellypy.CONFIG.FACEBOOK_TOKEN == 'temp':
            return {'result': 'waiting'}
        elif jellypy.CONFIG.FACEBOOK_TOKEN:
            token = jellypy.CONFIG.FACEBOOK_TOKEN
    ...
    def import_database(self, app=None, database_file=None, database_path=None, method=None, backup=False,
        ...
        if database_file:
            database_path = os.path.join(jellypy.CONFIG.CACHE_DIR, database_file.filename + '.import.db')
    ...
    def import_config(self, config_file=None, config_path=None, backup=False, **kwargs):
        ...
        if config_file:
            config_path = os.path.join(jellypy.CONFIG.CACHE_DIR, config_file.filename + '.import.ini')
    ...
    @cherrypy.tools.json_out()
    @requireAuth(member_of("admin"))
    def get_jellypy_jellyfin_token(self, username=None, password=None, force=False, **kwargs):
        ...
        # TODO: Jellyfin
        # plex_tv = plextv.PlexTV(username=username, password=password)
        # token = plex_tv.get_jellypy_jellyfin_token(force=force)
        token = None
    ...
    def check_login(self, hostname=None, port=None, ssl=0, remote=0, manual=0, user=None, password=None,
        ...
        """ Try Jellyfin Login.
            ...
            ```
        """
        # Attempt to get the jellyfin_identifier from plex.tv if the server is published
    ...
    def get_server_info(self, **kwargs):
        """ Get the JELLYFIN server information.
            ...
            Returns:
                json:
                    {"jellyfin_identifier": "08u2phnlkdshf890bhdlksghnljsahgleikjfg9t",
                     "jellyfin_ip": "10.10.10.1",
                     "jellyfin_is_remote": 0,
                     "jellyfin_name": "Winterfell-Server",
                     "jellyfin_platform": "Windows",
                     "jellyfin_plexpass": 1,
                     "jellyfin_port": 32400,
                     "jellyfin_ssl": 0,
                     "jellyfin_url": "http://10.10.10.1:32400",
                     "jellyfin_url_manual": 0,
                     "jellyfin_version": "1.20.0.3133-fede5bdc7"
                    }
            ```
        """
        # TODO: Jellyfin
        # server = plextv.get_server_resources(return_info=True)
        # server.pop('jellyfin_is_cloud', None)
        # return server
    ...
    def get_server_pref(self, pref=None, **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_server_pref(pref=pref)
        #
        # if result:
        #     return result
        # else:
    ...
    def update_check(self, **kwargs):
        ...
        if jellypy.UPDATE_AVAILABLE is None:
            update = {'result': 'error',
                      'update': None,
                      'message': 'You are running an unknown version of Tautulli.'
                      }

        elif jellypy.UPDATE_AVAILABLE == 'release':
            update = {'result': 'success',
                      'update': True,
                      'release': True,
                      'message': 'A new release (%s) of Tautulli is available.' % jellypy.LATEST_RELEASE,
                      'current_release': jellypy.common.RELEASE,
                      'latest_release': jellypy.LATEST_RELEASE,
                      'release_url': helpers.anon_url(
                          'https://github.com/%s/%s/releases/tag/%s'
                          % (jellypy.CONFIG.GIT_USER,
                             jellypy.CONFIG.GIT_REPO,
                             jellypy.LATEST_RELEASE))
                      }

        elif jellypy.UPDATE_AVAILABLE == 'commit':
            update = {'result': 'success',
                      'update': True,
                      'release': False,
                      'message': 'A newer version of Tautulli is available.',
                      'current_version': jellypy.CURRENT_VERSION,
                      'latest_version': jellypy.LATEST_VERSION,
                      'commits_behind': jellypy.COMMITS_BEHIND,
        ...
        if jellypy.DOCKER or jellypy.SNAP or jellypy.FROZEN:
            update['install_type'] = jellypy.INSTALL_TYPE
    ...
    def do_state_change(self, signal, title, timer, **kwargs):
        ...
        if jellypy.CONFIG.HTTP_ROOT.strip('/'):
            new_http_root = '/' + jellypy.CONFIG.HTTP_ROOT.strip('/') + '/'
        else:
            new_http_root = '/'
    ...
    def update(self, **kwargs):
        if jellypy.PYTHON2:
            raise cherrypy.HTTPRedirect(jellypy.HTTP_ROOT + "home?update=python2")
        if jellypy.DOCKER or jellypy.SNAP:
            raise cherrypy.HTTPRedirect(jellypy.HTTP_ROOT + "home")

        # Show changelog after updating
        jellypy.CONFIG.__setattr__('UPDATE_SHOW_CHANGELOG', 1)
        jellypy.CONFIG.write()
    ...
    def checkout_git_branch(self, git_remote=None, git_branch=None, **kwargs):
        if git_branch == jellypy.CONFIG.GIT_BRANCH:
            logger.error("Already on the %s branch" % git_branch)
            raise cherrypy.HTTPRedirect(jellypy.HTTP_ROOT + "home")

        # Set the new git remote and branch
        jellypy.CONFIG.__setattr__('GIT_REMOTE', git_remote)
        jellypy.CONFIG.__setattr__('GIT_BRANCH', git_branch)
        jellypy.CONFIG.write()
    ...
    def get_changelog(self, latest_only=False, since_prev_release=False, update_shown=False, **kwargs):
        ...
        if since_prev_release and jellypy.PREV_RELEASE == common.RELEASE:
            latest_only = True
            since_prev_release = False

        # Set update changelog shown status
        if helpers.bool_true(update_shown):
            jellypy.CONFIG.__setattr__('UPDATE_SHOW_CHANGELOG', 0)
            jellypy.CONFIG.write()
    ...
    def info(self, rating_key=None, guid=None, source=None, section_id=None, user_id=None, **kwargs):
        if rating_key and not str(rating_key):
            raise cherrypy.HTTPRedirect(jellypy.HTTP_ROOT)
        ...
        # Try to get metadata from the Jellyfin server first
        if rating_key:
            jellyfin_connect = jellyconnect.JellyConnect()
            metadata = jellyfin_connect.get_metadata_details(rating_key=rating_key, section_id=section_id)
    ...
    @cherrypy.expose
    @requireAuth()
    def get_item_children(self, rating_key="", media_type="", **kwargs):
        jellyfin_connect = jellyconnect.JellyConnect()
        result = jellyfin_connect.get_item_children(rating_key=rating_key, media_type=media_type)

        if result:
            return serve_template(templatename="info_children_list.html", data=result, media_type=media_type,
                                  title="Children List")
        else:
    ...
    def get_item_children_related(self, rating_key='', title='', **kwargs):
        jellyfin_connect = jellyconnect.JellyConnect()
        result = jellyfin_connect.get_item_children_related(rating_key=rating_key)
    ...
    def send_manual_on_created(self, notifier_id='', rating_key='', **kwargs):
        ...
        # if rating_key:
        #     jellyfin_connect = jellyconnect.JellyConnect()
        #     metadata = jellyfin_connect.get_metadata_details(rating_key=rating_key)
        ...
        #         children = jellyfin_connect.get_item_children(rating_key=rating_key)
    ...
    def image(self, *args, **kwargs):
        if args:
            ...
            if len(args) >= 2 and args[0] == 'images':
                resource_dir = os.path.join(str(jellypy.PROG_DIR), 'data/interfaces/default/')
                try:
    ...
    def download_config(self, **kwargs):
        ...
        return serve_download(jellypy.CONFIG_FILE, name=config_file)
    ...
    def download_database(self, **kwargs):
        ...
        try:
            ...
            shutil.copyfile(jellypy.DB_FILE, os.path.join(jellypy.CONFIG.CACHE_DIR, database_file))
            db.connection.rollback()
        except:
            pass

        return serve_download(os.path.join(jellypy.CONFIG.CACHE_DIR, database_file), name=database_file)
    ...
    def download_log(self, logfile='', **kwargs):
        ...
        return serve_download(os.path.join(jellypy.CONFIG.LOG_DIR, filename), name=filename)
    ...
    def delete_cache(self, folder='', **kwargs):
        ...
        cache_dir = os.path.join(jellypy.CONFIG.CACHE_DIR, folder)
        result = 'success'
    ...
    def search_results(self, query='', limit='', **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_search_results(query=query, limit=limit)
        #
        # if result:
        #     return result
        # else:
    ...
    def update_metadata_details(self, old_rating_key, new_rating_key, media_type, **kwargs):
        ...
        #     new_key_list = jellyfin_connect.get_rating_keys_list(rating_key=new_rating_key, media_type=media_type)
        #
    ...
    def get_new_rating_keys(self, rating_key='', media_type='', **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_rating_keys_list(rating_key=rating_key, media_type=media_type)
        #
        # if result:
        #     return result
        # else:
    ...
    def get_jellyfin_sessions_json(self, **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_sessions('json')
        #
        # if result:
        #     return result
        # else:
        #     logger.warn("Unable to retrieve data for get_jellyfin_sessions_json.")
        #     return False
    ...
    def get_recently_added_details(self, start='0', count='0', media_type='', section_id='', **kwargs):
        ...
        jellyfin_connect = jellyconnect.JellyConnect()
        result = jellyfin_connect.get_recently_added_details(start=start, count=count, media_type=media_type,
    ...
    def get_servers(self, **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_server_list(output_format='json')
        #
        # if result:
        #     return result
        # else:
    ...
    def get_servers_info(self, **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_servers_info()
        #
        # if result:
        #     return result
        # else:
    ...
    @requireAuth(member_of("admin"))
    @addtoapi()
    def login_to_jellyfin(self, hostname=None, port=None, ssl=0, username=None, password=None):
        """ Login to Jellyfin with username and password.
            ```
            Required parameters:
                username (str):     'admin'
        ...
        request_handler = http_handler.HTTPHandler(urls=url, ssl_verify=False)
        request = request_handler.make_request(uri=uri, request_type='POST', json=json_, output_format='json')

        if request is not None:
            return request
        else:
            logger.warn('Unable to login to Jellyfin. Please check your credentials.')
            return request

    @cherrypy.expose
    @requireAuth(member_of("admin"))
    # @addtoapi()
    def create_jellyfin_api_key(self, hostname=None, port=None, ssl=0, access_token=None):
        """ Login to Jellyfin with username and password.
            ```
            Required parameters:
                username (str):     'admin'
        ...
        scheme = 'https' if ssl else 'http'
        url = '{scheme}://{hostname}:{port}'.format(scheme=scheme, hostname=hostname, port=port)
        uri = f'/Auth/Keys?app=JellyPy-v{jellypy.version.JELLYPY_VERSION}'

        request_handler = http_handler.HTTPHandler(urls=url, ssl_verify=False, access_token=access_token)
        request = request_handler.make_request(uri=uri, request_type='POST')

        api_key = self.fetch_api_key(hostname, port, ssl, access_token)

        if request is not None:
            logger.info('Successfully created API Key in Jellyfin.')
            return api_key
        else:
            logger.error('Unable to create API Key in Jellyfin.')

    @requireAuth(member_of("admin"))
    @cherrypy.tools.json_out()
    def fetch_api_key(self, hostname=None, port=None, ssl=0, access_token=None):
        ssl = helpers.bool_true(ssl)
        scheme = 'https' if ssl else 'http'
        url = '{scheme}://{hostname}:{port}'.format(scheme=scheme, hostname=hostname, port=port)
        uri = f'/Auth/Keys'
        ...
        request_handler = http_handler.HTTPHandler(urls=url, ssl_verify=False, access_token=access_token)
        request = request_handler.make_request(uri=uri, request_type='GET')

        if request['TotalRecordCount'] != 0:
            api_key = request['Items'][0]['AccessToken']
            if request is not None:
                logger.info('Successfully created API Key in Jellyfin.')
                return api_key
            else:
    ...
    @requireAuth(member_of("admin"))
    @addtoapi()
    def get_server_id(self, hostname=None, port=None, identifier=None, ssl=0, get_url=False, api_key=None,
                      **kwargs):
        ...
        if request:
            ...
            # url = f"{scheme}://{request['LocalAddress']}:{request['WebSocketPortNumber']}"
    ...
    def get_server_identity(self, **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_server_identity()
        #
        # if result:
        #     return result
        # else:
    ...
    def get_server_friendly_name(self, **kwargs):
        """ Get the name of the JELLYFIN.
            ...
            ```
        """
        result = jellyconnect.get_server_friendly_name()
    ...
    def get_full_libraries_list(self, **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_library_details()
        #
        # if result:
        #     return result
        # else:
    ...
    def get_sync_transcode_queue(self, **kwargs):
        ...
        # TODO: Jellyfin
        # jellyfin_connect = jellyconnect.JellyConnect()
        # result = jellyfin_connect.get_sync_transcode_queue(output_format='json')
        #
        # if result:
        #     return result
        # else:
    ...
    @cherrypy.tools.json_out()
    @requireAuth(member_of("admin"))
    def get_jellypy_url(self, **kwargs):
        return helpers.get_jellypy_url()
    ...
    def newsletter(self, *args, **kwargs):
        ...
        if jellypy.CONFIG.NEWSLETTER_AUTH == 2:
            redirect_uri = request_uri.replace('/newsletter', '/newsletter_auth')
            raise cherrypy.HTTPRedirect(redirect_uri)

        elif jellypy.CONFIG.NEWSLETTER_AUTH == 1 and jellypy.CONFIG.NEWSLETTER_PASSWORD:
            if len(args) >= 2 and args[0] == 'image':
                return self.newsletter_auth(*args, **kwargs)
            elif kwargs.pop('key', None) == jellypy.CONFIG.NEWSLETTER_PASSWORD:
                return self.newsletter_auth(*args, **kwargs)
            else:
    ...
    def status(self, *args, **kwargs):
        ...
        if args or kwargs:
            if not cherrypy.request.path_info == '/api/v2' and jellypy.AUTH_ENABLED:
    ...
    def server_status(self, *args, **kwargs):
        ...
        """
        cherrypy.response.headers['Cache-Control'] = "max-age=0,no-cache,no-store"
        status = {'result': 'success', 'connected': jellypy.JELLYFIN_SERVER_UP}
