...
def check_active_sessions(ws_request=False):
    with monitor_lock:
        ...
        logger.debug("JellyPy Monitor :: Checking for active streams.")
        ...
        if session_list:
            ...
            for stream in db_streams:
                if any(d['session_key'] == str(stream['session_key']) and d['rating_key'] == str(stream['rating_key'])
                        ...
                        if session['session_key'] == str(stream['session_key']) and \
                            ...
                            if session['state'] != stream['state']:
                                if session['state'] == 'paused':
                                    logger.debug("JellyPy Monitor :: Session %s paused." % stream['session_key'])

                                    jellypy.NOTIFY_QUEUE.put({'stream_data': stream.copy(), 'notify_action': 'on_pause'})

                                if session['state'] == 'playing' and stream['state'] == 'paused':
                                    logger.debug("JellyPy Monitor :: Session %s resumed." % stream['session_key'])
                                ...
                                if session['state'] == 'error':
                                    logger.debug(
                                        "JellyPy Monitor :: Session %s encountered an error." % stream['session_key'])
                            ...
                            if session['state'] == 'buffering' and jellypy.CONFIG.BUFFER_THRESHOLD > 0:
                                ...
                                if buffer_values[0]['buffer_count'] >= jellypy.CONFIG.BUFFER_THRESHOLD:
                                    ...
                                    if buffer_values[0]['buffer_count'] == jellypy.CONFIG.BUFFER_THRESHOLD:
                                        logger.info("JellyPy Monitor :: User '%s' has triggered a buffer warning."
                                                    % stream['user'])
                                        ...
                                        if helpers.timestamp() > buffer_values[0]['buffer_last_triggered'] + \
                                            ...
                                            logger.info(
                                                "JellyPy Monitor :: User '%s' has triggered multiple buffer warnings."
                                                % stream['user'])
                                            # Set the buffer trigger time
                                            monitor_db.action('UPDATE sessions '
                                                              'SET buffer_last_triggered = strftime("%s","now") '
                                ...
                                logger.debug(
                                    "JellyPy Monitor :: Session %s is buffering. Count is now %s. Last triggered %s."
                                    % (stream['session_key'],
                    ...
                    if stream['state'] != 'stopped':
                        logger.debug("JellyPy Monitor :: Session %s stopped." % stream['session_key'])
                    ...
                    if row_id:
                        ...
                        logger.debug("JellyPy Monitor :: Removing sessionKey %s ratingKey %s from session queue"
                                     % (stream['session_key'], stream['rating_key']))
                        ...
                        if stream['write_attempts'] < jellypy.CONFIG.SESSION_DB_WRITE_ATTEMPTS:
                            logger.warn(
                                "JellyPy Monitor :: Failed to write sessionKey %s ratingKey %s to the database. " \
            ...
            for session in media_container:
                ...
                if new_session:
                    logger.debug("JellyPy Monitor :: Session %s started by user %s (%s) with ratingKey %s (%s)%s."
            ...
            logger.debug("JellyPy Monitor :: Unable to read session list.")
...
def connect_server(log=True, startup=False):
    ...
    if status:
        if log and not startup:
            logger.info("JellyPy Monitor :: Attempting to reconnect Plex server...")
...
def check_server_updates():
    with monitor_lock:
        logger.info("JellyPy Monitor :: Checking for PMS updates...")
        ...
        #
        # if download_info:
        #     logger.info("JellyPy Monitor :: Current PMS version: %s", jellypy.CONFIG.PMS_VERSION)
        #
        #     if download_info['update_available']:
        #         logger.info("JellyPy Monitor :: PMS update available version: %s", download_info['version'])
        #
        #         jellypy.NOTIFY_QUEUE.put({'notify_action': 'on_pmsupdate', 'pms_download_info': download_info})
        #
        #     else:
        #         logger.info("JellyPy Monitor :: No PMS update available.")
