...
def datetime_to_unix(date):
    try:
        date_format = datetime.datetime.strptime(date[:23], "%Y-%m-%dT%H:%M:%S.%f")
    except ValueError:
        try:
            date_format = datetime.datetime.strptime(date[:22], "%Y-%m-%dT%H:%M:%S.%f")
        except ValueError:
            date_format = datetime.datetime.strptime(date[:21], "%Y-%m-%dT%H:%M:%S.%f")
...
def convert_ticks(ticks):
    ...
    converted_ticks = converted_ticks.total_seconds()
...
def get_json_attr(json_key, attribute, return_bool=False, default_return=''):
    """
    Validate xml keys to make sure they exist and return their attribute value, return blank value is none found
    """
    try:
        if json_key[attribute]:
            if return_bool:
                return True
            else:
                return json_key[attribute]
        else:
            if return_bool:
                return False
            else:
...
def sanitize_ids(ids):
    res = ''.join([i for i in ids if not i.isdigit()])
...
def anon_url(*url):
    ...
    """
    if jellypy.CONFIG.ANON_REDIRECT_DYNAMIC:
        cache_time = timestamp()
        cache_filepath = os.path.join(jellypy.CONFIG.CACHE_DIR, 'anonymizer.json')
        try:
            ...
            try:
                ...
                anon_redirect = jellypy.CONFIG.ANON_REDIRECT
    else:
        anon_redirect = jellypy.CONFIG.ANON_REDIRECT
...
def get_jellypy_url(hostname=None):
    if jellypy.CONFIG.ENABLE_HTTPS:
        scheme = 'https'
    else:
        scheme = 'http'

    if hostname is None and jellypy.CONFIG.HTTP_HOST in ('0.0.0.0', '::'):
        import socket
        try:
        ...
        if not hostname:
            hostname = 'localhost'
    elif hostname == 'localhost' and jellypy.CONFIG.HTTP_HOST not in ('0.0.0.0', '::'):
