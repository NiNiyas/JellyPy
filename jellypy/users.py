...
def refresh_users():
    logger.info("JellyPy Users :: Requesting users list refresh...")
    result = jellyfin.Jellyfin().get_full_users_list()

    server_id = jellypy.CONFIG.JELLYFIN_IDENTIFIER
    if not server_id:
        logger.error("JellyPy Users :: Jellyfin identifier not found, cannot refresh users. Verify server in settings.")
        return

    if not jellypy.CONFIG.JELLYFIN_API_KEY:
        logger.error("JellyPy Users :: Jellyfin API Key not found, cannot refresh users. Verify server in settings.")
        return
    ...
    if result:
        ...
        logger.info("JellyPy Users :: Users list refreshed.")
        return True
    else:
        logger.warn("JellyPy Users :: Unable to refresh users list.")
        return False
...
class Users(object):
    ...
    def get_datatables_list(self, kwargs=None, grouping=None):
        ...
        try:
            ...
            logger.warn(f"JellyPy Users :: Unable to execute database query for get_list: {e}")
    ...
    def get_datatables_unique_ips(self, user_id=None, kwargs=None):
        ...
        try:
            ...
            logger.warn("JellyPy Users :: Unable to execute database query for get_unique_ips: %s." % e)
            return default_return
    ...
    def set_config(self, user_id=None, friendly_name='', custom_thumb='', do_notify=1, keep_history=1, allow_guest=1):
        if str(user_id):
            ...
            try:
                ...
                logger.warn("JellyPy Users :: Unable to execute database query for set_config: %s." % e)
    ...
    def get_details(self, user_id=None, user=None, email=None, include_last_seen=False):
        ...
        if user_details:
            ...
            logger.warn("JellyPy Users :: Unable to retrieve user %s from database. Requesting user list refresh."
            ...
            if user_details:
                ...
                logger.warn("JellyPy Users :: Unable to retrieve user %s from database. Returning 'Local' user."
                            % user_id if user_id else user)
    ...
    def get_user_details(self, user_id=None, user=None, email=None, include_last_seen=False):
        ...
        try:
            ...
            logger.warn("JellyPy Users :: Unable to execute database query for get_user_details: %s." % e)
            result = []
    ...
    def get_watch_time_stats(self, user_id=None, grouping=None, query_days=None):
        ...
        for days in query_days:
            ...
            try:
                ...
                logger.warn("JellyPy Users :: Unable to execute database query for get_watch_time_stats: %s." % e)
                result = []
    ...
    def get_player_stats(self, user_id=None, grouping=None):
        ...
        try:
            ...
            logger.warn("JellyPy Users :: Unable to execute database query for get_player_stats: %s." % e)
            result = []
    ...
    def get_recently_watched(self, user_id=None, limit='10'):
        ...
        try:
            if str(user_id):
                query = f"SELECT session_history.id, session_history.media_type, guid, " \
                        f"session_history.rating_key, session_history.parent_rating_key, session_history.grandparent_rating_key, " \
                        f"title, parent_title, grandparent_title, original_title, " \
                        f"thumb, parent_thumb, grandparent_thumb, media_index, parent_media_index, " \
                        f"year, originally_available_at, added_at, live, started, user " \
                        f"FROM session_history_metadata " \
                        f"JOIN session_history ON session_history_metadata.id = session_history.id " \
                        f"WHERE user_id='{user_id}' " \
                        f"GROUP BY (CASE WHEN session_history.media_type = 'track' THEN session_history.parent_rating_key " \
                        f"ELSE session_history.rating_key END) " \
                        f"ORDER BY MAX(started) DESC LIMIT {limit}"
                result = monitor_db.select(query)
            else:
                result = []
        except Exception as e:
            logger.warn(f"JellyPy Users :: Unable to execute database query for get_recently_watched: {e}.")
            result = []
    ...
    def get_users(self):
        ...
        try:
            ...
            logger.warn("JellyPy Users :: Unable to execute database query for get_users: %s." % e)
            return []
    ...
    def delete(self, user_id=None, row_ids=None, purge_only=False):
        ...
        if row_ids and row_ids is not None:
            ...
            if purge_only:
                ...
                logger.info("JellyPy Users :: Deleting user with user_id %s from database."
                            % user_id)
                try:
                    ...
                    logger.warn("JellyPy Users :: Unable to execute database query for delete: %s." % e)
    ...
    def undelete(self, user_id=None, username=None):
        ...
        try:
            if user_id and str(user_id):
                ...
                if result:
                    logger.info("JellyPy Users :: Re-adding user with id %s to database." % user_id)
                    monitor_db.action('UPDATE users '
                ...
                if result:
                    logger.info("JellyPy Users :: Re-adding user with username %s to database." % username)
                    monitor_db.action('UPDATE users '
                                      'SET deleted_user = 0, keep_history = 1, do_notify = 1 '
            ...
            logger.warn("JellyPy Users :: Unable to execute database query for undelete: %s." % e)
    ...
    def get_user_names(self, kwargs=None):
        ...
        try:
            ...
            logger.warn("JellyPy Users :: Unable to execute database query for get_user_names: %s." % e)
            return None
    ...
    def get_filters(self, user_id=None):
        ...
        try:
            ...
            logger.warn("JellyPy Users :: Unable to execute database query for get_filters: %s." % e)
            result = {}
    ...
    def set_user_login(self, user_id=None, user=None, user_group=None, ip_address=None, host=None,
        ...
        if user_id is None or str(user_id):
            ...
            try:
                ...
                logger.warn("JellyPy Users :: Unable to execute database query for set_login_log: %s." % e)
    ...
    def clear_user_login_token(self, jwt_token=None, row_ids=None):
        ...
        if jwt_token:
            logger.debug("JellyPy Users :: Clearing user JWT token.")
            try:
                monitor_db.action('UPDATE user_login SET jwt_token = NULL '
                                  'WHERE jwt_token = ?',
                                  [jwt_token])
            except Exception as e:
                logger.error("JellyPy Users :: Unable to clear user JWT token: %s.", e)
                return False

        elif row_ids and row_ids is not None:
            row_ids = list(map(helpers.cast_to_int, row_ids.split(',')))
            logger.debug("JellyPy Users :: Clearing JWT tokens for row_ids %s.", row_ids)
            try:
                monitor_db.action('UPDATE user_login SET jwt_token = NULL '
                                  'WHERE id in ({})'.format(','.join(['?'] * len(row_ids))),
                                  row_ids)
            except Exception as e:
                logger.error("JellyPy Users :: Unable to clear JWT tokens: %s.", e)
                return False
    ...
    def get_datatables_user_login(self, user_id=None, jwt_token=None, kwargs=None):
        ...
        try:
            ...
            logger.warn("JellyPy Users :: Unable to execute database query for get_datatables_user_login: %s." % e)
            return default_return
    ...
    def delete_login_log(self):
        ...
        try:
            logger.info("JellyPy Users :: Clearing login logs from database.")
            monitor_db.action('DELETE FROM user_login')
            monitor_db.action('VACUUM')
            return True
        except Exception as e:
            logger.warn("JellyPy Users :: Unable to execute database query for delete_login_log: %s." % e)
            return False
