...
def initialize(config_file):
    with INIT_LOCK:
        ...
        try:
            ...
            print(e)
            alert_message('Failed to start JellyPy: Config file is corrupted.\n\n%s' % config_file)
            raise SystemExit("Unable to initialize JellyPy due to a corrupted config file. Exiting...")
        ...
        logger.info(f"Starting JellyPy {common.RELEASE}")

        logger.info(
            f"{build}{common.PLATFORM} {common.PLATFORM_RELEASE} ({common.PLATFORM_VERSION}{' - {}'.format(common.PLATFORM_LINUX_DISTRO) if common.PLATFORM_LINUX_DISTRO else ''})")

        logger.info(f"{str(SYS_TIMEZONE)} (UTC{SYS_UTC_OFFSET})")

        logger.info("Python {}".format(
            sys.version.replace('\n', '')
        ))

        logger.info(f"Program Dir: {PROG_DIR}")
        ...
        if CONFIG.JELLYFIN_DEVICE_ID == '' or not CONFIG.JELLYFIN_DEVICE_ID:
            logger.debug("Generating Jellyfin Device ID...")
            CONFIG.JELLYFIN_DEVICE_ID = generate_uuid()
            CONFIG.write()
...
def initialize_scheduler():
    ...
    with SCHED_LOCK:
        ...
        pms_update_check_hours = CONFIG.JELLYFIN_UPDATE_CHECK_INTERVAL if 1 <= CONFIG.JELLYFIN_UPDATE_CHECK_INTERVAL else 24
        ...
        schedule_job(database.optimize_db, 'Optimize JellyPy database',
                     hours=24, minutes=0, seconds=0)
        schedule_job(database.make_backup, 'Backup JellyPy database',
                     hours=backup_hours, minutes=0, seconds=0, args=(True, True))
        schedule_job(config.make_backup, 'Backup JellyPy config',
                     hours=backup_hours, minutes=0, seconds=0, args=(True, True))

        if WS_CONNECTED and CONFIG.JELLYFIN_IP and CONFIG.JELLYFIN_API_KEY:
            ...
            schedule_job(activity_pinger.check_server_updates, 'Check for Jellyfin updates',
                         hours=pms_update_check_hours * bool(CONFIG.MONITOR_JELLYFIN_UPDATES), minutes=0, seconds=0)
            ...
            schedule_job(users.refresh_users, 'Refresh users list', hours=user_hours, minutes=0, seconds=0)
            schedule_job(libraries.refresh_libraries, 'Refresh libraries list', hours=library_hours, minutes=0, seconds=0)

            schedule_job(activity_pinger.connect_server, 'Check for server response', hours=0, minutes=0, seconds=0)
            schedule_job(web_socket.send_ping, 'Websocket ping',
                         hours=0, minutes=0, seconds=10 * bool(CONFIG.WEBSOCKET_MONITOR_PING_PONG))

        else:
            # Cancel all jobs
            #schedule_job(plextv.get_server_resources, 'Refresh Plex server URLs' hours=0, minutes=0, seconds=0)
            ...
            schedule_job(activity_pinger.connect_server, 'Check for server response', hours=0, minutes=0, seconds=30, args=(False,))
            schedule_job(web_socket.send_ping, 'Websocket ping', hours=0, minutes=0, seconds=0)
...
def startup_refresh():
    ...
    #if CONFIG.JELLYFIN_API_KEY and CONFIG.JELLYFIN_IP and CONFIG.JELLYFIN_PORT:
    ...
    if CONFIG.JELLYFIN_API_KEY and CONFIG.REFRESH_USERS_ON_STARTUP:
        users.refresh_users()

    # Refresh the libraries list on startup
    if CONFIG.JELLYFIN_IP and CONFIG.JELLYFIN_API_KEY and CONFIG.REFRESH_LIBRARIES_ON_STARTUP:
...
def dbcheck():
    ...
    c_db.execute(
        ...
        'on_extup INTEGER DEFAULT 0, on_intup INTEGER DEFAULT 0, on_jellyupdate INTEGER DEFAULT 0, '
        'on_concurrent INTEGER DEFAULT 0, on_newdevice INTEGER DEFAULT 0, on_jellypyupdate INTEGER DEFAULT 0, '
        'on_jellypydbcorrupt INTEGER DEFAULT 0, '
        ...
        'on_extup_subject TEXT, on_intup_subject TEXT, on_jellyupdate_subject TEXT, '
        'on_concurrent_subject TEXT, on_newdevice_subject TEXT, on_jellypyupdate_subject TEXT, '
        'on_jellypydbcorrupt_subject TEXT, '
        ...
        'on_extup_body TEXT, on_intup_body TEXT, on_jellyupdate_body TEXT, '
        'on_concurrent_body TEXT, on_newdevice_body TEXT, on_jellypyupdate_body TEXT, '
        'on_jellypydbcorrupt_body TEXT, '
    ...
    # Upgrade notifiers table from earlier versions
    try:
        c_db.execute('SELECT on_jellypydbcorrupt FROM notifiers')
    except sqlite3.OperationalError:
        logger.debug("Altering database. Updating database table notifiers.")
        c_db.execute(
            'ALTER TABLE notifiers ADD COLUMN on_jellypydbcorrupt INTEGER DEFAULT 0'
        )
        c_db.execute(
            'ALTER TABLE notifiers ADD COLUMN on_jellypydbcorrupt_subject TEXT'
        )
        c_db.execute(
            'ALTER TABLE notifiers ADD COLUMN on_jellypydbcorrupt_body TEXT'
        )
...
def shutdown(restart=False, update=False, checkout=False, reset=False):
    ...
    if update:
        logger.info("JellyPy is updating...")
        try:
            versioncheck.update()
        except Exception as e:
            logger.warn("JellyPy failed to update: %s. Restarting." % e)

    if checkout:
        logger.info("JellyPy is switching the git branch...")
        try:
            versioncheck.checkout_git_branch()
        except Exception as e:
            logger.warn("JellyPy failed to switch git branch: %s. Restarting." % e)

    if reset:
        logger.info("JellyPy is resetting the git install...")
        try:
            versioncheck.reset_git_install()
        except Exception as e:
            logger.warn("JellyPy failed to reset git install: %s. Restarting." % e)
    ...
    if restart:
        logger.info("JellyPy is restarting...")
        ...
        if NOFORK:
            ...
            logger.info("Restarting JellyPy with %s", args)
        ...
        logger.info("JellyPy is shutting down...")
...
def initialize_tracker():
    ...
    tracker = Tracker.create('UA-111522699-2', client_id=CONFIG.JELLYFIN_UUID, hash_client_id=True,
